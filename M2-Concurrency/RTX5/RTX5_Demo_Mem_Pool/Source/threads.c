...
void Error_Flasher(void) {
	while (1) {					// error code on LEDs - fast red flashing
	...
	while (1) {
		...
		if (SWITCH_PRESSED(SW1_POS)) { 
			...
			if (cur_step < MAX_NUM_LED_STEPS) {
				...
				if (count < 9) { // R G Y etc.  
					pattern->Step[cur_step].R = (count & 1)? 1 : 0;
					pattern->Step[cur_step].G = (count & 2)? 1 : 0;
					pattern->Step[cur_step].B = (count & 4)? 1 : 0;
				} else { // R B R B etc.
					pattern->Step[cur_step].R = (count & 1)? 1 : 0;
					pattern->Step[cur_step].G = 0;
					pattern->Step[cur_step].B = (count & 1)? 0 : 1;
				}
				cur_step++;
			} else {
				Error_Flasher();
			}
			Control_RGB_LEDs(0, 1, 0); // Green flash satus indicator
