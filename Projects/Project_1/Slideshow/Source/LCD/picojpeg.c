...
static PJPG_INLINE uint8 getByte()
{	
	return getBits(8, 1);
}
//------------------------------------------------------------------------------
...
//#define USE_ORIGINAL_HUFF_DECODE
#define USE_GET_2_BITS_HUFF_DECODE
//#define USE_FAST_HUFF_DECODE

#pragma anon_unions
#ifdef USE_ORIGINAL_HUFF_DECODE
static uint8 huffDecode(const HuffTable* pHuffTable, const uint8* pHuffVal)
{
   uint8 i = 0; 
   uint8 j;
	
   uint16 code = getBit(); // get first bit of the code
   ...
   for ( ; ; )
   {
      uint16 maxCode;
		 ...
		 // Is our code less than the max code for this number
		 // of bits? If it is, then we found the index for the
		 // value table
      maxCode = pHuffTable->mMaxCode[i]; 
      if ((code <= maxCode) && (maxCode != 0xFFFF))
         break;

			// No, so the code must be longer. Shift in next bit of code.
      i++;
      code <<= 1;
      code |= getBit();		
...
typedef struct
{
	union
	{
		...
		struct
		{
			uint16_t _2bitLeftovers : 14; // bits 0:13
			uint16_t _2bitCode      : 2;  // bits 14:15
		};
		struct
		{
			uint16_t _3bitLeftOvers  : 13; // bits 0:13
			uint16_t _3bitCode       : 3;  // bits 14:15
		};
				struct
		{
			uint16_t _4bitLeftOvers  : 12; // bits 0:13
			uint16_t _4bitCode       : 4;  // bits 14:15
		};
				struct
		{
			uint16_t _5bitLeftOvers  : 11; // bits 0:13
			uint16_t _5bitCode       : 5;  // bits 14:15
		};
				struct
		{
			uint16_t _6bitLeftOvers  : 10; // bits 0:13
			uint16_t _6bitCode       : 6;  // bits 14:15
		};
				struct
		{
			uint16_t _7bitLeftOvers  : 9; // bits 0:13
			uint16_t _7bitCode       : 7;  // bits 14:15
		};
				struct
		{
			uint16_t _8bitLeftOvers  : 8; // bits 0:13
			uint16_t _8bitCode       : 8;  // bits 14:15
		};
				struct
		{
			uint16_t _9bitLeftOvers  : 7; // bits 0:13
			uint16_t _9bitCode       : 9;  // bits 14:15
		};
				struct
		{
			uint16_t _10bitLeftOvers  : 6; // bits 0:13
			uint16_t _10bitCode       : 10;  // bits 14:15
		};
				struct
		{
			uint16_t _11bitLeftOvers  : 5; // bits 0:13
			uint16_t _11bitCode       : 11;  // bits 14:15
		};
		struct
		{
			uint16_t _12bitLeftOvers  : 4; // bits 0:13
			uint16_t _12bitCode       : 12;  // bits 14:15
		};
		struct
		{
			uint16_t _13bitLeftOvers  : 3; // bits 0:13
			uint16_t _13bitCode       : 13;  // bits 14:15
		};
		struct
		{
			uint16_t _14bitLeftOvers  : 2; // bits 0:13
			uint16_t _14bitCode       : 14;  // bits 14:15
		};
		struct
		{
			uint16_t _15bitLeftOvers  : 1; // bits 0:13
			uint16_t _15bitCode       : 15;  // bits 14:15
		};
	};
} HuffBits_t;
static uint16_t codeArr[11] = {0};
// This is the last (bad) remaining of many failed iterations of this function
uint8_t i = 16;
uint8_t j = 0;
uint16 nextBits = 0;
uint16_t code;
uint8_t bitsDone = 0;
static HuffBits_t bits = {0};
_Bool firstTime = 1;
static uint16_t determineHuffCode(const HuffTable* pHuffTable, uint8_t* i);
static uint8_t huffDecode(const HuffTable* pHuffTable, const uint8* pHuffVal)
{
  //uint8 i j;	
	if (firstTime)
	{
    bits._8bitCode = getByte();
	//  bits._8bitLeftOvers = getByte();
		firstTime = 0;
	}
	else
	{
		// Replenish the bits
		bits.raw |= getBits(bitsDone, 1);
		bitsDone = 0;
	}

	code = determineHuffCode(pHuffTable, &i);	
...
{
	uint16_t code = 0;
	if (bits._2bitCode <= pHuffTable->mMaxCode[1] && pHuffTable->mMaxCode[1] != 0xffff)
	{
		*i = 1;
		code = bits._2bitCode;
	}
	else if (bits._3bitCode <= pHuffTable->mMaxCode[2] && pHuffTable->mMaxCode[2] != 0xffff)
	{
		*i = 2;
		code = bits._3bitCode;
	}
	else if (bits._4bitCode <= pHuffTable->mMaxCode[3] && pHuffTable->mMaxCode[3] != 0xffff)
	{
		*i = 3;
		code = bits._4bitCode;
	}
	else if (bits._5bitCode <= pHuffTable->mMaxCode[4] && pHuffTable->mMaxCode[4] != 0xffff)
	{
		*i = 4;
		code = bits._5bitCode;
	}
	else if (bits._6bitCode <= pHuffTable->mMaxCode[5] && pHuffTable->mMaxCode[5] != 0xffff)
	{
		*i = 5;
		code = bits._6bitCode;
	}
	else if (bits._7bitCode <= pHuffTable->mMaxCode[6] && pHuffTable->mMaxCode[6] != 0xffff)
	{
		*i = 6;
		code = bits._7bitCode;
	}
	else if (bits._8bitCode <= pHuffTable->mMaxCode[7] && pHuffTable->mMaxCode[7] != 0xffff)
	{
		*i = 7;
		code = bits._8bitCode;
	}
	else if (bits._9bitCode <= pHuffTable->mMaxCode[8] && pHuffTable->mMaxCode[8] != 0xffff)
	{
		*i = 8;
		code = bits._9bitCode;
	}
	else if (bits._10bitCode <= pHuffTable->mMaxCode[9] && pHuffTable->mMaxCode[9] != 0xffff)
	{
		*i = 9;
		code = bits._10bitCode;
	}
	else if (bits._11bitCode <= pHuffTable->mMaxCode[10] && pHuffTable->mMaxCode[10] != 0xffff)
	{
		*i = 10;
		code = bits._11bitCode;
	}
	else if (bits._12bitCode <= pHuffTable->mMaxCode[11] && pHuffTable->mMaxCode[11] != 0xffff)
	{
		*i = 11;
		code = bits._12bitCode;
	}
	else if (bits._13bitCode <= pHuffTable->mMaxCode[12] && pHuffTable->mMaxCode[12] != 0xffff)
	{
		*i = 12;
		code = bits._13bitCode;
	}
	else if (bits._14bitCode <= pHuffTable->mMaxCode[13] && pHuffTable->mMaxCode[13] != 0xffff)
	{
		*i = 13;
		code = bits._14bitCode;
	}
	else if (bits._15bitCode <= pHuffTable->mMaxCode[14] && pHuffTable->mMaxCode[14] != 0xffff)
	{
		*i = 14;
		code = bits._15bitCode;
	}
	else if (bits.raw <= pHuffTable->mMaxCode[15] && pHuffTable->mMaxCode[15] != 0xffff)
	{
		*i = 15;
		code = bits.raw;
	}
...
#ifdef USE_GET_2_BITS_HUFF_DECODE
static uint8_t huffDecode(const HuffTable* pHuffTable, const uint8* pHuffVal)
{
   uint8 i2 = 1;
   uint8 j2;
	
   // We know code is at least 2 bits wide and max of 15
   uint16 code2 = getBits(2, 1); // get first bits of the code

   for ( ; ; )
   {		 
		  uint16 maxCode2;
      maxCode2 = pHuffTable->mMaxCode[i2]; 
      if ((code2 <= maxCode2) && (maxCode2 != 0xFFFF))
			{
         break;
			}
	 ...
	 j2 = pHuffTable->mValPtr[i2];
	 j2 = (uint8)(j2 + (code2 - pHuffTable->mMinCode[i2])); 
