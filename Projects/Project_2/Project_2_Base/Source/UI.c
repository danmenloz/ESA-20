...
UI_FIELD_T Fields[] = {
	...
	&green, &black, 1, 0, 1, 0, Control_OnOff_Handler},	
	{"T_flash_prd ", "ms", "", (volatile int *)&g_flash_period, NULL, {0,10}, 
	&green, &black, 1, 0, 1, 0, Control_IntNonNegative_Handler},		
...
int ave_samples(volatile uint16_t *buff) {
	unsigned int sum=0;
	for (uint8_t i=0;i<SAMPLES_PER_PIXEL;i++) sum+=buff[i];
...
#pragma O0
#endif
void UI_Draw_Scope(int trig_sample) {
	PT_T tp1,tp2;
	int start_idx=0;            	// start of display (trigger-PRE_TRIG_SAMPLES)/4
	int setY,idx=0;

	DEBUG_START(DBG_DRAWSCOPE);
#if PARTIAL_SCOPE_ERASE
	// Clear the scope area.
	tp1.X=0;	tp1.Y=0;
	tp2.X=LCD_WIDTH/2;	tp2.Y=g_scope_height;
	LCD_Fill_Rectangle(&tp1,&tp2,&black);
#else
	// Clear the scope area.
	tp1.X=0;	tp1.Y=0;
	tp2.X=LCD_WIDTH;	tp2.Y=g_scope_height;
	LCD_Fill_Rectangle(&tp1,&tp2,&black);
#endif 
	if (g_holdoff > 0) {
		// Draw a red line to mark the trigger point.
		tp1.X=g_holdoff>>2;	tp1.Y=0;
		tp2.X=g_holdoff>>2;	tp2.Y=g_scope_height;
		LCD_Draw_Line(&tp1,&tp2,&red);
	}
	// Determine the sample index of the start of the display
	// This is g_holdoff samples before the trigger accounting for
	// sample buffer wrapping.
	start_idx=trig_sample-g_holdoff;
	if (start_idx < 0) start_idx+=NUM_CURR_SAMPLES;
	
	// Loop through NUM_CURR_SAMPLES of samples
	for (int x=0; x < NUM_CURR_SAMPLES; x+=SAMPLES_PER_PIXEL) {
	#if PARTIAL_SCOPE_ERASE
		if (x == LCD_WIDTH/2) {
			// Clear the scope area.
			tp1.X=LCD_WIDTH/2;	tp1.Y=0;
			tp2.X=LCD_WIDTH;	tp2.Y=g_scope_height;
			LCD_Fill_Rectangle(&tp1,&tp2,&black);
		}
	#endif
		idx=(start_idx+x)%NUM_CURR_SAMPLES;	// active buffer position Shift by start_idx and wrap		
		tp1.X=(x/SAMPLES_PER_PIXEL);				// displayX is sampleX/4

#if DEBUG_SCOPE
		if (idx == trig_sample) {						// Exists to set a debug point in.
			tp2.X=(start_idx+x)%NUM_CURR_SAMPLES;
		}
#endif
		// plot set point.
		setY=ave_samples(&g_set_sample[idx]);
		tp1.Y=SCALE_SCOPE(setY);
		LCD_Plot_Pixel(&tp1,&light_blue);
		
		// plot from old set point to new set point. (x is same as set point)
		setY=SCALE_MEASURED(ave_samples(&g_meas_sample[idx]));
		tp1.Y=SCALE_SCOPE(setY);		// Compute ave 4-point value
		LCD_Plot_Pixel(&tp1,&yellow);
	}
	DEBUG_STOP(DBG_DRAWSCOPE);
}
#if DEBUG_SCOPE
#pragma pop
#endif
...
#if DRAW_SCOPE
	if (g_disp_buffer) {
		UI_Draw_Scope(g_trigger_pt);
		g_trigger_pt=TRIG_PEND;				// display complete; clear status.
	}
#else
